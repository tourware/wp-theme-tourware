...
abstract class AbstractVueListing extends Widget
{
    ...
    protected function render( $instance = [] )
    {
        ...
        if ($settings['pagi'] == 'load_more') {
            $config['load_more_text'] = $settings['pagination_button_text'];
        }

        ob_start();
        Icons_Manager::render_icon( $settings['style_duration_icon'] );
        $icon_days = ob_get_contents();
        ob_end_clean();
        $config['icon_days'] = $icon_days;

        ob_start();
        Icons_Manager::render_icon( $settings['style_persons_icon'] );
        $icon_persons = ob_get_contents();
        ob_end_clean();
        $config['icon_persons'] = $icon_persons;

        ob_start();
        Icons_Manager::render_icon( $settings['style_destination_icon'] );
        $icon_destination = ob_get_contents();
        ob_end_clean();
        $config['icon_destination'] = $icon_destination;

        if ($settings['show_read_more']) {
            $config['read_more'] = true;
        ...
        while ( $query->have_posts() ):
            ...
            $posts[] = self::getListItemData($record, $settings);
        endwhile;
...
//        $data['optional_attributes_html'] = $settings[ 'open_new_tab' ] === 'yes' ? 'target="_blank"' : '';
        ...
        if ($settings['show_price']) {
            ...
            $data['price'] = $settings['price_prefix'].number_format($price, 0, ',', '.').$settings['price_suffix'];
        }
        ...
        if ($settings['show_persons']) {
            $persons = ($item_data->getPaxMin() ? $item_data->getPaxMin().'-' : '').$item_data->getPaxMax();
            if ($persons) $data['persons'] = $persons.$settings['persons_suffix'];
        }
                ...
                if ($score['score_tags']) {
                    foreach ($tags as $tag) {
                        if (in_array($tag->name, $score['score_tags'])) {
                            ob_start();
                            Icons_Manager::render_icon( $score['score_icon'] );
                            echo '<div class="text">'.$tag->name.'</div>';
